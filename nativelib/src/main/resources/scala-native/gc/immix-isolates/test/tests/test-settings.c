...
MunitResult parse_sizeStr(const MunitParameter params[], void *user_data_or_fixture) {
  ...
  munit_assert_size(Settings_parseSizeStr(SIZE_MAX_STR), ==, UNLIMITED_HEAP_SIZE);
  SIZE_MAX_STR[str_end] = 'K';
  munit_assert_size(Settings_parseSizeStr(SIZE_MAX_STR), ==, UNLIMITED_HEAP_SIZE);
